<?php
/**
 * Bedigas: An extensible micro-framework.
 *
 * @copyright   Copyright (c) 2011, Mike Cao <arjuna@arjunacoding.com>
 * @license     MIT, http://bedigasphp.com/license
 */

namespace bedigas;

use bedigas\core\Loader;
use bedigas\core\Dispatcher;
 ...
 * @method \bedigas\template\View view() Gets current view
 *
 * Request-response
...
class Engine {
    ...
    public function init() {
        ...
        $this->loader->register('request', '\bedigas\net\Request');
        $this->loader->register('response', '\bedigas\net\Response');
        $this->loader->register('router', '\bedigas\net\Router');
        $this->loader->register('view', '\bedigas\template\View', array(), function($view) use ($self) {
            $view->path = $self->get('bedigas.views.path');
            $view->extension = $self->get('bedigas.views.extension');
        });
        ...
        $this->set('bedigas.base_url', null);
        $this->set('bedigas.case_sensitive', false);
        $this->set('bedigas.handle_errors', true);
        $this->set('bedigas.log_errors', false);
        $this->set('bedigas.views.path', './views');
        $this->set('bedigas.views.extension', '.php');

        // Startup configuration
        $this->before('start', function() use ($self) {
            // Enable error handling
            if ($self->get('bedigas.handle_errors')) {
                set_error_handler(array($self, 'handleError'));
                set_exception_handler(array($self, 'handleException'));
            }

            // Set case-sensitivity
            $self->router()->case_sensitive = $self->get('bedigas.case_sensitive');
        });
    ...
    public function handleException($e) {
        if ($this->get('bedigas.log_errors')) {
    ...
    public function _redirect($url, $code = 303) {
        $base = $this->get('bedigas.base_url');
